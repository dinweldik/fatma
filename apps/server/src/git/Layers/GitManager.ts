...
} from "@fatma/shared/git";

import { GitManagerError } from "@fatma/contracts";
import {
  GitManager,
...
function resolvePullRequestWorktreeLocalBranchName(
  ...
  return `fatma/pr-${pullRequest.number}/${suffix}`;
}
          ...
          Effect.flatMap((suggestion) =>
            suggestion
              ...
              : Effect.fail(
                  gitManagerError(
                    "generateCommitMessage",
                    "No staged changes are available to summarize.",
                  ),
                ),
          ),
        ),
          ...
          ? yield* progress
              ...
              .pipe(
                ...
                Effect.map((result) => ({
                  ...
                  ...(result.upstreamBranch !== null
                    ? { upstreamBranch: result.upstreamBranch }
                    : {}),
                  ...(result.setUpstream !== undefined ? { setUpstream: result.setUpstream } : {}),
                })),
              )
          : { status: "skipped_not_requested" as const };
