...
const ServerConfigLayer = ServerConfig.layerTest(process.cwd(), { prefix: "fatma-git-core-test-" });
  ...
  describe("renameGitBranch", () => {
    ...
    it.effect("appends numeric suffix when target branch already exists", () =>
      Effect.gen(function* () {
        const tmp = yield* makeTmpDir();
        yield* initRepoWithCommit(tmp);
        yield* (yield* GitCore).createBranch({ cwd: tmp, branch: "fatma/feat/session" });
        yield* (yield* GitCore).createBranch({ cwd: tmp, branch: "fatma/tmp-working" });
        yield* (yield* GitCore).checkoutBranch({ cwd: tmp, branch: "fatma/tmp-working" });
        ...
        const branches = yield* (yield* GitCore).listBranches({ cwd: tmp });
        expect(branches.branches.some((branch) => branch.name === "fatma/feat/session")).toBe(true);
    ...
    it.effect("increments suffix until it finds an available branch name", () =>
      Effect.gen(function* () {
        ...
        yield* initRepoWithCommit(tmp);
        yield* (yield* GitCore).createBranch({ cwd: tmp, branch: "fatma/feat/session" });
        yield* (yield* GitCore).createBranch({ cwd: tmp, branch: "fatma/feat/session-1" });
        yield* (yield* GitCore).createBranch({ cwd: tmp, branch: "fatma/tmp-working" });
        yield* (yield* GitCore).checkoutBranch({ cwd: tmp, branch: "fatma/tmp-working" });
    ...
    it.effect(
      ...
      () =>
        Effect.gen(function* () {
          ...
          yield* git(tmp, ["remote", "add", "origin", "git@github.com:pingdotgg/fatma.git"]);

          const remoteName = yield* core.ensureRemote({
            cwd: tmp,
            preferredName: "origin",
            url: "git@github.com:pingdotgg/fatma.git/",
          });
    ...
    it.effect(
      ...
      () =>
        Effect.gen(function* () {
          ...
          expect(
            yield* git(tmp, ["ls-remote", "--heads", "jasonLaster", "fatma/pr-488/statemachine"]),
          ).toBe("");
        }),
    );
