...
const WsRpcLayer = WsRpcGroup.toLayer(
  Effect.gen(function* () {
    ...
    return WsRpcGroup.of({
      ...
      [WS_METHODS.serverUpdateTelegramNotifications]: (input) =>
        telegramNotifications.updateSettings(input).pipe(
          Effect.mapError(
            (cause) =>
              new ServerTelegramNotificationsError({
                detail: cause.detail,
              }),
          ),
        ),
      [WS_METHODS.serverSendTestTelegramNotification]: (input) =>
        telegramNotifications.sendTestNotification(input).pipe(
          Effect.mapError(
            (cause) =>
              new ServerTelegramNotificationsError({
                detail: cause.detail,
      ...
      [WS_METHODS.projectsBrowseDirectory]: (input) =>
        workspaceFileSystem.browseDirectory(input).pipe(
          Effect.mapError((cause) => {
            const message = Schema.is(WorkspacePathOutsideRootError)(cause)
              ? "Workspace path must stay within the project root."
              : cause.detail;
            return new ProjectBrowseDirectoryError({
              message,
              cause,
            });
          }),
      ...
      [WS_METHODS.gitPush]: (input) => git.pushCurrentBranch(input.cwd, null),
      [WS_METHODS.gitCommit]: (input) => git.commitStaged(input),
      [WS_METHODS.gitReadWorkingTreeFileDiff]: (input) => git.readWorkingTreeFileDiff(input),
      [WS_METHODS.gitStageFiles]: (input) => git.stageFiles(input),
      [WS_METHODS.gitUnstageFiles]: (input) => git.unstageFiles(input),
      ...
      [WS_METHODS.gitGenerateCommitMessage]: (input) => gitManager.generateCommitMessage(input),
