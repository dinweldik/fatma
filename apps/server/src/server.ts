...
const OrchestrationNotificationLayerLive = Layer.effectDiscard(
  Effect.gen(function* () {
    ...
    yield* Stream.runForEach(orchestrationEngine.streamDomainEvents, (event) =>
      Effect.gen(function* () {
        ...
        const thread =
          snapshot.threads.find((entry) => entry.id === event.payload.threadId) ?? null;
...
const runServerEffect = Layer.launch(makeServerLayer);
type RunServerError =
  typeof runServerEffect extends Effect.Effect<infer _A, infer E, infer _R> ? E : never;

export const runServer = runServerEffect as Effect.Effect<void, RunServerError, ServerConfig>;
