...
import { TelegramNotifications, type TelegramNotificationsShape } from "./telegramNotifications.ts";
...
const buildAppUnderTest = (options?: {
  ...
  layers?: {
    ...
    telegramNotifications?: Partial<TelegramNotificationsShape>;
  };
}) =>
  Effect.gen(function* () {
    ...
    const appLayer = HttpRouter.serve(makeRoutesLayer, {
      ...
      Layer.provide(
        Layer.mock(TelegramNotifications)({
          getSettingsSummary: Effect.succeed({
            chatId: "",
            hasBotToken: false,
            botTokenHint: null,
            enabled: false,
          ...
          sendTestNotification: () => Effect.succeed({ delivered: false }),
          sendOrchestrationNotification: () => Effect.void,
          ...options?.layers?.telegramNotifications,
        }),
      ),
