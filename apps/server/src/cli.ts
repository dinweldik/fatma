...
const baseDirFlag = Flag.string("base-dir").pipe(
  Flag.withDescription("Base directory path (equivalent to FATMA_HOME)."),
  Flag.optional,
);
...
const EnvServerConfig = Config.all({
  logLevel: Config.logLevel("FATMA_LOG_LEVEL").pipe(Config.withDefault("Info")),
  mode: Config.schema(RuntimeMode, "FATMA_MODE").pipe(
    Config.option,
  ...
  bootstrapFd: Config.int("FATMA_BOOTSTRAP_FD").pipe(
    Config.option,
    Config.map(Option.getOrUndefined),
  ),
